...
def extraerMejorAlgoritmo1(texto):
  ...
  for line in iter(texto.splitlines()):
  	result = re.search("Numero veces algoritmo 1: (.*)", line)
  	if result:
...
def extraerMejorAlgoritmo2(texto):
  costo = []
  for line in iter(texto.splitlines()):
  	result = re.search("Numero veces algoritmo 2: (.*)", line)
  	if result:
  		costo.append(result.group(1))
  return costo
...
def extraerMejorAlgoritmo3(texto):
  costo = []
  for line in iter(texto.splitlines()):
  	result = re.search("Numero veces algoritmo 3: (.*)", line)
  	if result:
  		costo.append(result.group(1))
  return costo
...
def extraerMejorAlgoritmo4(texto):
  costo = []
  for line in iter(texto.splitlines()):
  	result = re.search("Numero veces algoritmo 4: (.*)", line)
  	if result:
  		costo.append(result.group(1))
  return costo
...
def main(): 
	...
	f = open("resultadosModelo3_sumatotal.csv", 'w')
